...
watch:
	$(FOP_FOLDER)/fopc -w -i data -i $(FOP_FOLDER)/data src $(FOP_FOLDER)/lib $(foreach backend,$(BACKENDS),$(FOP_FOLDER)/backends/$(backend)) -o $(OUT) $(FOPC_PARAMETERS)
...
ide:
	...
	$(eval PORT2 := $(shell curl https://srv.launix.de/p/register))
	...
	(cd $(FOP_FOLDER); ./fopide "$(shell pwd)/src" --data "$(shell pwd)/data" --data ./data $(foreach backend,$(BACKENDS),backends/$(backend)) ./lib -p$(PORT))
...
release.tar.xz: all
	(cd $(OUT) && tar cavf ../release.tar.xz --exclude=conf.json --exclude='fop-files/[0-9]*' . )
...
update:
	...
	curl -L http://srv.launix.de/fopc/fopc -o $(FOP_FOLDER)/fopc
	curl -L http://srv.launix.de/fopc/fopide -o $(FOP_FOLDER)/fopide
